...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkLtSymbol;
		else if (ev->x > selmon->ww - (int)TEXTW(stext) + TEXTW("\0"))
			click = ClkStatusText;
		else
			click = ClkWinTitle;
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - tw - x) > bh) {
		...
		if (m->sel) {
			...
			drw_text(drw, x, 0, tw, bh, lrpad/2, m->sel->name, 0);
			
			if (m->sel->isfloating)
				drw_rect(drw, x + boxs - (roundwin ? boxs*2 : 0), bh/2-boxw/2, boxw, boxw, m->sel->isfixed, 0);
		}
	}
